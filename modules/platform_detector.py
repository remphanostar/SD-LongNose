...
class PlatformDetector:
    ...
    def _detect_by_filesystem(self) -> Optional[str]:
        ...
        fs_markers = [
            ("/storage", "/notebooks", "paperspace"),
            ("/notebooks", "digitalocean"),
            ("/teamspace", "lightning"),
            ("/workspace", "vastai"),
            ("/opt/ml", "sagemaker"),
            ("/content", "colab"),
        ...
        for marker in fs_markers:
            if len(marker) == 2:
                path, platform = marker
                if os.path.exists(path):
                    # Check for exclusions
                    if platform == "digitalocean" and os.path.exists("/storage"):
                        continue
                    return platform
            elif len(marker) == 3:
                path1, path2, platform = marker
