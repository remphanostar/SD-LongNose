...
class UnifiedPinokioEngine:
    ...
    async def install_app(self, app_name: str, progress_callback=None) -> bool:
        ...
        try:
            ...
            # Get repository URL
            repo_url = app_info.get('reference', app_info.get('src'))
            if not repo_url:
                logger.error(f"No repository URL found for {app_name}")
                return False
            ...
            if not app_path.exists():
                ...
                try:
                    ...
                    logger.info(f"Cloned {app_name} from {repo_url}")
                except Exception as e:
                    logger.error(f"Failed to clone {app_name}: {e}")
                    return False
