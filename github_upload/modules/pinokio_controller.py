...
class PinokioController:
    ...
    def start_server(self, headless: bool = True, port: int = None) -> bool:
        ...
        try:
            ...
            pinokio_binary = self.get_pinokio_binary_path()
    ...
    def get_pinokio_binary_path(self) -> str:
        """Get path to Pinokio binary based on platform"""        
        import platform
        system = platform.system().lower()
        
        if system == "linux":
            # Check for AppImage files with dynamic naming
            ...
            # Default to Linux AppImage with dynamic detection
            if os.path.exists(self.pinokio_path):
                for file in os.listdir(self.pinokio_path):
                    if file.startswith("Pinokio") and file.endswith(".AppImage"):
                        return os.path.join(self.pinokio_path, file)
